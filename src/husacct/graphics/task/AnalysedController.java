...
public class AnalysedController extends DrawingController {
	...
	public void drawArchitecture(DrawingDetail detail) {
		...
		ArrayList<ProjectDTO> projects = controlService.getApplicationDTO().projects;
	...
	protected DependencyDTO[] getDependenciesBetween(BaseFigure figureFrom,	BaseFigure figureTo) {
		...
		if (!figureFrom.equals(figureTo) && null != dtoFrom && null != dtoTo) return analyseService.getDependencies(dtoFrom.uniqueName, dtoTo.uniqueName);
	...
	private void initializeServices() {
		...
		validateService = ServiceProvider.getInstance()
				.getValidateService();
		validateService.addServiceListener(new IServiceListener() {
			@Override
			public void update() {
				if (AnalysedController.this.areViolationsShown()) AnalysedController.this.refreshDrawing();
			}
		});
	}
	...
	public void moduleZoom(BaseFigure[] figures) {
		...
		for(BaseFigure fig : figures)
			if(!(fig instanceof ProjectFigure)) allProjects = false;
			else
				allProjects = true;
		
		if(allProjects){
			setCurrentPathsForProjects(figures);
			ProjectDTO project = (ProjectDTO) getFigureMap().getModuleDTO(figures[0]);
			AbstractDTO[] abstractDTOs = project.analysedModules.toArray(new AbstractDTO[project.analysedModules.size()] );
			if(abstractDTOs.length != 0) this.drawModulesAndLines(abstractDTOs);
		}
