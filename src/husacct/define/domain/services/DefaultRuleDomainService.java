...
public class DefaultRuleDomainService {
    ...
    //TODO: Needs to be revised when Validate is working (loop through list from the validate service)
    public boolean isMandatoryRule(String ruleTypeKey, Module moduleFrom) {
	if(ruleTypeKey.equals("IsNotAllowedToMakeSkipCall") || ruleTypeKey.equals("IsNotAllowedToMakeBackCall")){
	    if(moduleFrom.getType() == "Layer"){
		return true;
	    }
	}else if(ruleTypeKey.equals("VisibilityConvention")){
	    if(moduleFrom.getType().equals("Component") || moduleFrom.getType().equals("ExternalLibrary")){
		return true;
	    }
