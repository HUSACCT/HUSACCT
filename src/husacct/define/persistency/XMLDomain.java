...
public class XMLDomain {
	...
	private Project getProjectFromElement(Element XMLElement) {
		...
		project.setProgrammingLanguage(XMLElement.getChild(
		...
		List<Element> pathElements = XMLElement.getChild("paths").getChildren(
				"path");
	...
	private AppliedRuleStrategy getAppliedRuleFromXML(Element XMLElement) {
		...
		long newID = ruleService.addAppliedRule(dummyRule.getRuleType(),
				dummyRule.getDescription(), dummyRule.getDependencies(),
				dummyRule.getRegex(), dummyRule.getModuleFrom(),
				dummyRule.getModuleTo(), dummyRule.isEnabled());
		AppliedRuleStrategy newRule = ruleService.getAppliedRuleById(newID);
		newRule.setId(dummyRule.getId());
		return newRule;
	...
	// TODO: Check if this function really works
	private ArrayList<AppliedRuleStrategy> getExceptionsFromXML(
			Element XMLElement) {
		ArrayList<AppliedRuleStrategy> exceptions = new ArrayList<AppliedRuleStrategy>();
		for (Element exception : XMLElement.getChild("exceptions").getChildren(
				"AppliedRule")) {
...
// AppliedRuleStrategy AppliedXMLRule =
// ruleFactory.createRule(ruleType.getValue());
// AppliedXMLRule.setAppliedRule(ruleDescription.getValue(),
// dependencies.toArray(new String[dependencies.size()]),ruleRegex.getValue(),
...
// ArrayList<AppliedRuleStrategy> ruleList = new
// ArrayList<AppliedRuleStrategy>();
//
